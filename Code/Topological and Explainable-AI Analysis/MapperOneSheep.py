...
def load_data_with_labels():
    """加载6个通道数据并添加分层标签"""
    data_paths = [
        ("Data/SHAP/data/1A.xlsx", "077", "invasive"),
        ("Data/SHAP/data/1B.xlsx", "077", "non-invasive_left"),
        ("Data/SHAP/data/1C.xlsx", "077", "non-invasive_right")
    ]
    ...
    for path, sheep, signal_type in data_paths:
        ...
        # 统一特征列名为f0-f35格式
        feature_columns = [f"f{i}" for i in range(36)]  # 36个特征列
        df.columns = feature_columns + list(df.columns[36:])  # 保留原始额外列
        
        # 添加元数据
        df = df.iloc[:, :36]  # 确保只取前36个特征列
        df['sheep'] = sheep
